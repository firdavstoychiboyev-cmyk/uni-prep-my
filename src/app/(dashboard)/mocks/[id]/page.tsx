...
export default function MockTestPage() {
    ...
    const [passages, setPassages] = useState<Record<string, PassageDoc>>({});
        ...
        return (
            <div className="max-w-2xl mx-auto px-6 py-16 flex flex-col items-center text-center gap-6">
                ...
                <div className="w-full mt-6 text-left">
                    ...
                    <MockReview questions={questions} answers={answers} passages={passages} />
                </div>
            </div>
        );
    }
    ...
    const currentPassage = q?.passageId ? passages[q.passageId] ?? null : null;

    // Shared question+options JSX — used in both single-column and split layouts
    const questionPane = (
        <>
            <div
    ...
    return (
        <>
            <div className="fixed inset-0 z-50 bg-background flex flex-col">
                ...
                {/* Question area — two-column split when passage attached */}
                {currentPassage ? (
                    <div className="flex-1 flex overflow-hidden">
                        {/* Left: passage (stays stable across questions that share the same passage) */}
                        <div className="w-[46%] shrink-0 overflow-y-auto border-r border-border bg-muted/20">
                            <div className="px-6 py-8 flex flex-col gap-6">
                                {currentPassage.title && (
                                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest border-b border-border pb-3">
                                        {currentPassage.title}
                                    </p>
                                )}
                                {currentPassage.blocks.map((block, bi) => (
                                    <div key={bi} className="flex flex-col gap-2">
                                        ...
                                        <p
                                            ...
                                            style={{ fontFamily: "var(--font-source-serif), Georgia, serif", lineHeight: "1.85" }}
                                        >
                                            {block.text}
                                        </p>
                                    </div>
